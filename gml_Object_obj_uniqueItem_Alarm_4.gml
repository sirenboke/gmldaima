...
show_debug_message("Relic Drop Amount: " + string(_relicsAmount));
var _rewardText = string(amount) + "x " + string(name) + " obtained by: ";
...
for (var ii = 0; ii < array_length(_rewardedPlayers); ii++)
{
    with (obj_player)
    {
        if (_rewardedPlayers[ii] == id)
        {
            ...
            show_debug_message("Relic Drop " + string(i) + " earned by " + string(name) + ".");
            
            if (string_pos(name, other.rewardText) == 0)
                other.rewardText += (string(name) + ", ");
...
with (obj_player)
{
    ...
    if (_myRewards > 0)
    {
        _rewardText += (string(name) + " (" + string(_myRewards) + "x, " + string(gotUniqueItem[other.itemID]) + "x owned)! ");
...
rewardText = string_delete(rewardText, string_length(rewardText) - 1, 2);
rewardText += ")";
